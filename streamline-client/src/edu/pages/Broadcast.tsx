...
import { reconnectMedia } from "../../lib/mediaRecovery";
...
export default function Broadcast() {
  ...
  useEffect(() => {
    ...
    if (isBypass) {
      // Demo mode: use mock org settings
      setOrg({
        id: "edu-demo-org",
        name: "EDU Demo",
  ...
  const activeCamIdRef = useRef<string>("");
  const activeMicIdRef = useRef<string>("");
  ...
  async function stopDeviceStreams() {
    ...
    activeCamIdRef.current = "";
    activeMicIdRef.current = "";
  ...
  async function ensureCameraStream() {
    // If we already have a stream for the selected camera, skip.
    if (streamsRef.current?.cam && activeCamIdRef.current === cameraId) return;
    // If we have a stream for a DIFFERENT camera, stop it first.
    if (streamsRef.current?.cam && activeCamIdRef.current !== cameraId) {
      for (const t of streamsRef.current.cam.getTracks()) t.stop();
      streamsRef.current.cam = undefined;
      activeCamIdRef.current = "";
    }
    try {
      const cam = await navigator.mediaDevices.getUserMedia({
  ...
  // --- Auto-restart preview when user changes the camera dropdown (pre-live only) ---
  useEffect(() => {
    if (isLive) return;        // while live, switching is handled separately
    if (!cameraId) return;     // no device selected yet
    if (!streamsRef.current?.cam) return; // no preview running — user hasn't clicked Test yet
    if (activeCamIdRef.current === cameraId) return; // same device, nothing to do

    // Stop the old camera track, acquire the new one, and attach to preview
    let cancelled = false;
    (async () => {
      const oldCam = streamsRef.current?.cam;
      if (oldCam) {
        for (const t of oldCam.getTracks()) t.stop();
      }
      try {
        const cam = await navigator.mediaDevices.getUserMedia({
        ...
        if (!cancelled) setDeviceError(String(e?.message || "Unable to access camera"));
      }
    })();
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // --- Switch camera while live (via LiveKit room.switchActiveDevice) ---
  const [switchingDevice, setSwitchingDevice] = useState(false);

  async function switchLiveCamera(newCamId: string) {
    if (!newCamId || switchingDevice) return;
    const room = lkRoomRef.current;
    if (!room) {
      setDeviceError("LiveKit room not connected");
      return;
    }
    ...
    try {
      if (room.switchActiveDevice) {
        await room.switchActiveDevice("videoinput", newCamId);
      } else {
        // Fallback: use reconnectMedia helper
        await reconnectMedia(room, { videoDeviceId: newCamId });
      }
      setCameraId(newCamId);
      // Re-attach preview to the live video element
      ...
      setDeviceError(String(e?.message || "Failed to switch camera"));
    } finally {
      setSwitchingDevice(false);
    }
  }

  async function switchLiveMic(newMicId: string) {
    if (!newMicId || switchingDevice) return;
    const room = lkRoomRef.current;
    if (!room) {
      setDeviceError("LiveKit room not connected");
      return;
    ...
    try {
      if (room.switchActiveDevice) {
        ...
        await reconnectMedia(room, { audioDeviceId: newMicId });
      }
      setMicId(newMicId);
    } catch (e: any) {
      setDeviceError(String(e?.message || "Failed to switch microphone"));
    } finally {
      setSwitchingDevice(false);
    }
  }
  ...
  async function startBroadcast() {
    ...
    if (isEduBypassEnabled()) {
      // Demo mode: simulate live state without connecting to LiveKit
      setIsLive(true);
      setStartedAt(Date.now());
      setBroadcastId("demo_broadcast_" + Date.now());
  ...
  async function endBroadcast() {
    ...
    // Stop server-side (skip in demo mode)
    if (broadcastId && !isEduBypassEnabled()) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          {/* Live Device Switcher */}
          <div className="rounded-2xl border border-slate-800/50 bg-slate-900/50 p-6">
            <div className="mb-4">
              <div className="text-lg font-semibold text-white">Devices</div>
              <div className="mt-1 text-sm text-slate-400">Switch camera or mic while live.</div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="mt-2 w-full rounded-lg border border-slate-700/60 bg-slate-950/40 px-3 py-2 text-sm text-slate-200 outline-none focus:border-orange-500/40 disabled:opacity-50"
                >
                  {cameraDevices.length ? (
                    cameraDevices.map((d) => (
                      <option key={d.deviceId} value={d.deviceId}>
              ...
              <button
                type="button"
                disabled={switchingDevice}
                onClick={() => refreshDevices().catch(() => void 0)}
                className="w-full rounded-lg border border-slate-800 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-50"
              >
                {switchingDevice ? "Switching…" : "Refresh Devices"}
              </button>

              {deviceError ? <div className="text-sm text-red-300">{deviceError}</div> : null}
            </div>
          </div>
        </div>
      </div>
