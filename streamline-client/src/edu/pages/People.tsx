...
export default function People() {
  ...
  useEffect(() => {
    ...
    if (isBypass) {
      // Demo mode: use mock people
      const demoPeople: EduPerson[] = [
        { id: "demo-p1", name: "Ms. Johnson", email: "johnson@school.edu", role: "faculty_admin", status: "active", lastActiveAt: new Date().toISOString() },
        { id: "demo-p2", name: "Alex Rivera", email: "alex.r@school.edu", role: "student_producer", status: "active", lastActiveAt: new Date(Date.now() - 3600_000).toISOString() },
        { id: "demo-p3", name: "Jordan Kim", email: "jordan.k@school.edu", role: "student_producer_assigned", status: "active", lastActiveAt: new Date(Date.now() - 7200_000).toISOString() },
        { id: "demo-p4", name: "Sam Chen", email: "sam.c@school.edu", role: "talent", status: "active", lastActiveAt: new Date(Date.now() - 86400_000).toISOString() },
        { id: "demo-p5", name: "Taylor Davis", email: "taylor.d@school.edu", role: "talent", status: "invited", lastActiveAt: null },
        { id: "demo-p6", name: "Casey Morgan", email: "casey.m@school.edu", role: "viewer", status: "active", lastActiveAt: new Date(Date.now() - 172800_000).toISOString() },
        { id: "demo-p7", name: "Riley Green", email: "riley.g@school.edu", role: "viewer", status: "active", lastActiveAt: null },
      ];
  ...
  async function onDisable(p: EduPerson) {
    ...
    if (isBypass) {
      setPeople((cur) => cur.map((x) => x.id === p.id ? { ...x, status: "disabled" } : x));
      return;
    }
  ...
  async function onResend(p: EduPerson) {
    ...
    if (isBypass) return; // no-op in demo
  ...
  async function onSaveRole() {
    ...
    try {
      if (isBypass) {
        setPeople((cur) => cur.map((x) => x.id === editTarget.id ? { ...x, role: editRole } : x));
        setEditOpen(false);
        setEditTarget(null);
      } else {
